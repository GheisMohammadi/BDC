...
func (r *Record) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	...
	if len(r.Key) > 0 {
		i -= len(r.Key)
		copy(dAtA[i:], r.Key)
		i = encodeVarintIntrospection(dAtA, i, uint64(len(r.Key)))
		i--
		dAtA[i] = 0xa
	}
	// Value []byte
	if len(r.Value) > 0 {
		i -= len(r.Value)
		copy(dAtA[i:], r.Value)
		i = encodeVarintIntrospection(dAtA, i, uint64(len(r.Value)))
		i--
		dAtA[i] = 0xa
	}
	// TimeReceived         string 
	if len(r.TimeReceived) > 0 {
		i -= len(r.TimeReceived)
		copy(dAtA[i:], r.TimeReceived)
		i = encodeVarintIntrospection(dAtA, i, uint64(len(r.TimeReceived)))
		i--
		dAtA[i] = 0x12
	}
	...
	if len(r.XXX_unrecognized) > 0 {
		i -= len(r.XXX_unrecognized)
		copy(dAtA[i:], r.XXX_unrecognized)
		i = encodeVarintIntrospection(dAtA, i, uint64(len(r.XXX_unrecognized)))
		i--
		dAtA[i] = 0xa
	}
	// XXX_sizecache        int32
	if r.XXX_sizecache != 0 {
		i = encodeVarintIntrospection(dAtA, i, uint64(r.XXX_sizecache))
		i--
		dAtA[i] = 0x50
	}
